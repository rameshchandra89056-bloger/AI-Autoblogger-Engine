...
def ask_ai(prompt, retries=3): # Retries badha diye hain
    for i in range(retries):
        ...
        try:
            ...
            time.sleep(5) # Delay takki rate limit reset ho
            
    print("🔄 Switching to Pollinations API Backup...")
    try:
        poll_url = "https://text.pollinations.ai/"
        res_poll = requests.post(poll_url, json={"messages": [{"role": "system", "content": "Tum ek expert Hindi blogger ho. Sirf HTML output do."}, {"role": "user", "content": prompt}], "model": "openai"}, timeout=40)
...
current_topic = ""
blog_content = ""
emergency_topics = ["2026 Mein AI Se Paise Kaise Kamaye: Ek Pura Guide", "Top 5 Share Market Tips Jo Apko Pata Honi Chahiye", "Bina Investment Ke Online Business Kaise Shuru Karein"]
...
try:
    ...
    # --- THE SUPERVISOR FIX: DELAYS AND EMERGENCY FALLBACKS ---
    print("⏳ Chunk 1 generating...")
    intro_prompt = f"Topic: '{current_topic}'. Ek lamba Introduction likho jo kisi anjaan (beginner) ko bhi samajh aa jaye. Uske baad ek Clickable TOC likho. Niyam: TOC mein aise links banao: <ul style='list-style:none; padding:0;'><li>👉 <a href='#basic' style='color:#da251c; text-decoration:none; font-weight:bold;'>1. Basic Samajh</a></li> <li>👉 <a href='#deep' style='color:#da251c; text-decoration:none; font-weight:bold;'>2. Deep Detail</a></li> <li>👉 <a href='#pro' style='color:#da251c; text-decoration:none; font-weight:bold;'>3. Pro Hacks</a></li></ul>. Sirf HTML code do (<h2>, <p>, <a>)."
    chunk_1 = ask_ai(intro_prompt)
    if not chunk_1: chunk_1 = f"<h2>{current_topic} ki Shuruaat</h2><p>Aaj ke digital yug mein, sahi jankari hi sabse badi taqat hai. Is article mein hum aapko detail mein samjhayenge ki kaise aap apne aarthik lakshyon ko prapt kar sakte hain. Aaiye shuru karte hain aur dekhte hain isme aapke liye kya-kya hai.</p><div style='background: #fffafa; border-left: 5px solid #da251c; padding: 20px; border-radius: 8px; margin-bottom: 25px;'><h3 style='color: #da251c; margin-top: 0;'>📍 Is Article Mein Kya Hai:</h3><ul style='list-style:none; padding:0;'><li>👉 <a href='#basic' style='color:#da251c; text-decoration:none; font-weight:bold;'>1. Basic Samajh</a></li><li>👉 <a href='#deep' style='color:#da251c; text-decoration:none; font-weight:bold;'>2. Deep Detail</a></li><li>👉 <a href='#pro' style='color:#da251c; text-decoration:none; font-weight:bold;'>3. Pro Hacks</a></li></ul></div>"
    
    time.sleep(5) # The Cool-down Protocol
    
    print("⏳ Chunk 2 generating...")
    body_prompt = f"Topic: '{current_topic}'. Ab main body likho. Theek 3 sub-headings (<h2>) likho. Niyam 1: Pehle <h2> mein id='basic' lagao. Dusre <h2> mein id='deep' lagao. Teesre <h2> mein id='pro' lagao. (Jaise: <h2 id='basic'>Basic Samajh</h2>). Niyam 2: Har section lamba hona chahiye aur example ke sath hona chahiye. Niyam 3: Har <h2> wale section ke baad exactly 1 baar [PHOTO] tag likho. Sirf HTML code do."
...
send_telegram_msg(urllib.parse.quote(f"✅ FINAL SUCCESS: Unbreakable Blog Live!\n📝 Kul Post: {len(posts_db)}\n🌐 Link: {blog_url}"))
