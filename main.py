...
# --- ADVANCED MULTI-LAYER AI ENGINE (ULTIMATE GOD MODE) ---
def ask_ai(prompt, retries=2):
    # Plan A: Google Gemini Flash (Stable v1beta Endpoint)
    for i in range(retries):
        ...
        try:
            # Safety settings added back to prevent 400 errors on Finance keywords
            payload_data = {
            ...
            # Plan B: Google Gemini Pro immediately if Flash fails
            if e.code == 404 or e.code == 403:
                ...
                try:
                    ...
                    with urllib.request.urlopen(req_pro, timeout=30) as res_pro:
                        ...
                        if len(text_pro) > 10: return text_pro
    ...
    # Plan C: Pollinations Text API (THE MASTERSTROKE - No API Key Required)
    print("🔄 Switching to Pollinations Text API Backup...")
    try:
        poll_url = "https://text.pollinations.ai/"
        poll_payload = {
            "messages": [
                {"role": "system", "content": "Tum ek expert Hindi blogger aur storyteller ho. Sirf HTML code output do. ```html jaise markdown tag mat lagana."},
        ...
        if res_poll.status_code == 200:
            ...
            if len(text_poll) > 10: return text_poll
        else:
            print(f"⚠️ Pollinations Error: {res_poll.status_code}")
    except Exception as e:
...
current_topic = ""
blog_content = ""
emergency_topics = ["2026 Mein AI Se Paise Kaise Kamaye: Secret Hacks", "Top 5 Share Market Tips Jo Apko Pata Honi Chahiye", "Bina Investment Ke Online Business Kaise Shuru Karein"]
...
try:
    ...
    # --- ADVANCED PROMPT CHUNKING WITH DOST/STORY LOGIC ---
    print("⏳ Chunk 1 generating...")
    intro_prompt = f"Topic: '{current_topic}'. Tum ek expert Hindi blogger aur storyteller ho. Niyam 1: Kitabi bhasha ka bilkul prayog mat karna. Niyam 2: Shuruaat ek bohot hi chhoti aur dilchasp kahani (short story) ya kisi aam insaan ke real-life example se karo jo reader ko hook kar le. Niyam 3: Dost ki tarah asaan bhasha mein samjhao. Uske baad ek TOC (Table of Contents) likho. Template for TOC: <div style='background: #fffafa; border-left: 5px solid #da251c; padding: 20px; border-radius: 8px; margin-bottom: 25px;'><h3 style='color: #da251c; margin-top: 0;'>📍 Is Article Mein Kya Hai:</h3><ul><li>👉 Point 1</li><li>👉 Point 2</li></ul></div>. Sirf HTML code do (<h2>, <p>). Conclusion mat likhna."
    chunk_1 = ask_ai(intro_prompt, retries=2)
    
    print("⏳ Chunk 2 generating...")
    body_prompt = f"Topic: '{current_topic}'. Ab article ki main body likho. 2 detailed sub-headings (<h2>) aur paragraphs (<p>). Niyam 1: Kitabi bhasha (bookish language) ka bilkul prayog mat karna. Aise samjhao jaise tum ek expert dost ho aur kisi ko chai ki tapri par baith kar aasan shabdon mein paise kamana sikha rahe ho. Har point ke sath ek chota udaharan (example) zaroor do. Niyam 2: Har section ke baad exactly 1 baar [PHOTO] tag likho (kul 2 [PHOTO] tags hone chahiye). Sirf HTML code do."
    chunk_2 = ask_ai(body_prompt, retries=2)
    
    print("⏳ Chunk 3 generating...")
    conclusion_prompt = f"Topic: '{current_topic}'. Ab ek strong aur friendly Conclusion aur 3 FAQ likho. Niyam 1: Pura HTML format. Niyam 2: Beech mein exactly 2 baar [AFFILIATE] tag likho."
...
for idx, mod in enumerate(["cinematic", "cyberpunk", "hyperrealistic"]):
    if "[PHOTO]" in blog_content:
        inner_img_url = f"[https://image.pollinations.ai/prompt/](https://image.pollinations.ai/prompt/){urllib.parse.quote(f'{safe_img_base} {mod}')}?width=800&height=400&nologo=true&seed={post_id + idx + 1}"
        pre_warm_image(inner_img_url)
        blog_content = blog_content.replace("[PHOTO]", f"<div style='text-align: center;'><img src='{inner_img_url}' onerror=\"this.onerror=null; this.src='[https://placehold.co/800x400/c00000/ffffff?text=AI+Finance](https://placehold.co/800x400/c00000/ffffff?text=AI+Finance)';\" style='width: 100%; border-radius: 12px; margin: 35px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.15); object-fit: cover;'></div>", 1)

main_img_url = f"[https://image.pollinations.ai/prompt/](https://image.pollinations.ai/prompt/){urllib.parse.quote(f'{safe_img_base} masterpiece')}?width=1200&height=600&nologo=true&seed={post_id}"
...
footer_html = f"""<footer style="margin-top: 40px; background: #111; padding: 40px 20px; text-align: center;"><div style="margin-bottom: 25px;"><p style="color: #ccc; font-size: 14px; margin-bottom: 15px; font-weight: bold; letter-spacing: 1px;">JOIN THE AI MILLIONAIRE COMMUNITY:</p><div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;"><a href="[https://www.youtube.com/@TheAIMillionaire-h5g](https://www.youtube.com/@TheAIMillionaire-h5g)" target="_blank" style="color: #FF0000; text-decoration: none; font-weight: bold; background: white; padding: 8px 15px; border-radius: 5px;">YouTube</a><a href="[https://t.me/digitalkamaihub_2026](https://t.me/digitalkamaihub_2026)" target="_blank" style="color: #0088cc; text-decoration: none; font-weight: bold; background: white; padding: 8px 15px; border-radius: 5px;">Telegram</a><a href="[https://www.instagram.com/aimillionaire_official](https://www.instagram.com/aimillionaire_official)" target="_blank" style="color: #E1306C; text-decoration: none; font-weight: bold; background: white; padding: 8px 15px; border-radius: 5px;">Instagram</a></div></div><div class="footer-links" style="margin-bottom: 20px;"><a href="about.html">About Us</a> | <a href="privacy.html">Privacy Policy</a> | <a href="terms.html">Terms</a> | <a href="disclaimer.html">Disclaimer</a> | <a href="contact.html">Contact</a></div><p style="margin-top:20px; font-size:13px; color: #888;">&copy; {current_year} Digital Kamai Hub. All Rights Reserved.</p></footer>"""

article_page = f"""<!DOCTYPE html><html lang="hi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{current_topic} - Digital Kamai Hub</title>{premium_css}{schema_markup}</head><body><div style="position: fixed; top: 0; left: 0; width: 100%; height: 5px; background-color: transparent; z-index: 9999;"><div id="smart-progress" style="height: 5px; background-color: #da251c; width: 0%; border-top-right-radius: 3px; border-bottom-right-radius: 3px;"></div></div><script>window.addEventListener('scroll', function() {{ var winScroll = document.body.scrollTop || document.documentElement.scrollTop; var height = document.documentElement.scrollHeight - document.documentElement.clientHeight; var scrolled = (winScroll / height) * 100; document.getElementById("smart-progress").style.width = scrolled + "%"; }}); function toggleAudio() {{ var audio = document.getElementById("premium-audio"); var btn = document.getElementById("floating-tts-btn"); if (audio.paused) {{ audio.play(); btn.innerHTML = "⏸️ Pause Audio"; }} else {{ audio.pause(); btn.innerHTML = "▶️ Play Audio"; }} }}</script>{header_html}<div class="container"><div class="article-box"><h1 style="color: #111; margin-bottom: 15px;">{current_topic}</h1><div style="color: #666; font-size: 14px; margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px; font-weight: bold;">Date: {today_date} | Author: Mohit (The AI Millionaire)</div><img src="{main_img_url}" onerror="this.onerror=null; this.src='[https://placehold.co/1200x600/da251c/ffffff?text=Digital+Kamai+Hub](https://placehold.co/1200x600/da251c/ffffff?text=Digital+Kamai+Hub)';" style="width: 100%; border-radius: 10px; margin-bottom: 25px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); object-fit: cover;"><div id="article-body">{blog_content}</div><div style="background: linear-gradient(135deg, #f9f9f9, #ffffff); padding: 30px; border-radius: 12px; border: 2px dashed #da251c; margin-top: 40px; text-align: center; box-shadow: 0 10px 30px rgba(218, 37, 28, 0.1);"><h3 style="color: #111; font-size: 22px; margin-top: 0; margin-bottom: 10px;">🔥 2026 ke Secret Hacks Seedhe Inbox Mein!</h3><p style="color: #555; font-size: 16px; margin-bottom: 20px;">'The AI Millionaire' ki VIP list join karein aur rozana 10X kamai ke naye tarike seekhein (Free).</p><form action="[https://formsubmit.co/ajax/rameshchandra89056@gmail.com](https://formsubmit.co/ajax/rameshchandra89056@gmail.com)" method="POST" style="display: flex; gap: 10px; max-width: 500px; margin: 0 auto; flex-wrap: wrap; justify-content: center;"><input type="hidden" name="_subject" value="Naya VIP Subscriber: Digital Kamai Hub!"><input type="hidden" name="_autoresponse" value="Digital Kamai Hub mein swagat hai! Jaldi hi naye hacks milenge."><input type="email" name="email" placeholder="Apna Email likhein..." required style="flex: 1; padding: 12px; border: 1px solid #ccc; border-radius: 8px; font-size: 16px; min-width: 200px;"><button type="submit" style="background: #111; color: white; border: none; padding: 12px 25px; font-weight: bold; border-radius: 8px; font-size: 16px; cursor: pointer;">Join Now 🚀</button></form></div><audio id="premium-audio" src="{audio_filename}"></audio><button id="floating-tts-btn" onclick="toggleAudio()" style="display: block; width: 100%; background: #da251c; color: white; border: none; padding: 15px; font-size: 18px; font-weight: bold; border-radius: 8px; cursor: pointer; margin-top: 30px; box-shadow: 0 4px 15px rgba(218,37,28,0.3);">▶️ Play Audio</button><div style="margin-top: 25px; display: flex; gap: 15px;"><button onclick="window.open('[https://api.whatsapp.com/send?text=Digital](https://api.whatsapp.com/send?text=Digital) Kamai Hub ka naya secret: ' + window.location.href, '_blank')" style="flex: 1; background: #25D366; color: white; border: none; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 16px; box-shadow: 0 4px 10px rgba(37,211,102,0.2);">💬 WhatsApp</button><button onclick="window.open('[https://t.me/share/url?url=](https://t.me/share/url?url=)' + window.location.href + '&text=Digital Kamai Hub ka naya secret!', '_blank')" style="flex: 1; background: #0088cc; color: white; border: none; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 16px; box-shadow: 0 4px 10px rgba(0,136,204,0.2);">✈️ Telegram</button></div></div></div>{footer_html}</body></html>"""
...
blog_url = f"[https://rameshchandra89056-bloger.github.io/AI-Autoblogger-Engine/](https://rameshchandra89056-bloger.github.io/AI-Autoblogger-Engine/){post_filename}"
send_telegram_msg(urllib.parse.quote(f"✅ FINAL SUCCESS: Blog live!\n📝 Kul Post: {len(posts_db)}\n🌐 Link: {blog_url}"))
