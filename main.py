...
# ==========================================
# THE IMMORTAL SYSTEM - SOLAR PANEL EDITION (v31.0)
# Multi-API Key Rotation (Never Run Out of Quota)
# ==========================================

# 1. SOLAR PANEL LOGIC (सारी चाबियां कॉमा से अलग करके लोड करना)
...
print(f"☀️ सोलर पैनल एक्टिवेटेड: सिस्टम में {len(API_KEYS)} API Keys लोड हो गई हैं!")
...
# SMART MODEL HUNTER
# ---------------------------------------------------------
print("🔍 मॉडल ढूँढ रहे हैं...")
available_model = "models/gemini-1.5-flash"
list_url = f"https://generativelanguage.googleapis.com/v1beta/models?key={API_KEYS[0]}" # पहली key से चेक कर रहे हैं
try:
    ...
    with urllib.request.urlopen(req, timeout=30) as response:
        ...
        for m in res.get('models', []):
            name = m.get('name', '')
            if 'generateContent' in m.get('supportedGenerationMethods', []) and 'flash' in name.lower():
                available_model = name; break
except: pass

print(f"🎯 मॉडल सेट: {available_model}")

# ---------------------------------------------------------
# THE SOLAR-POWERED AI ENGINE (Key Rotation Loop)
# ---------------------------------------------------------
def ask_ai(prompt, retries=10):
    for i in range(retries):
        # यहाँ असली जादू है: हर कोशिश में अगली Key इस्तेमाल होगी! (Round Robin)
        ...
        try:
            ...
            print(f"⚠️ Key { (i % len(API_KEYS)) + 1 } फेल हो गई (Quota/Limit)। सोलर पैनल दूसरी Key पर शिफ्ट हो रहा है...")
            time.sleep(3) # बस 3 सेकंड रुककर दूसरी चाबी लगाएगा
        except Exception as e:
            print(f"⚠️ Network Error. शिफ्टिंग...")
            time.sleep(3)
    return ""
