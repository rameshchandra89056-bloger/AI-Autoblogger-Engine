...
# ==========================================
# THE IMMORTAL SYSTEM - PREMIUM UI/UX EDITION (v28.0)
# Multi-Agent Logic + Premium Responsive News Design + TTS
# ==========================================
...
blog_content = ask_ai(html_prompt, retries=10).replace("```html", "").replace("```", "").strip()
...
# 🎨 PREMIUM CSS DESIGN (Flipkart/Amazon/Tech Crunch Level)
premium_css = """
    <style>
...
<body>
    <header>
        ...
        <nav><a href="index.html">Home</a> <a href="#">News</a> <a href="#">Trending</a></nav>
    </header>

    <div class="main-container">
        <div class="article-header">
            <h1 class="article-title">{current_topic}</h1>
            <p class="meta-data">🗓️ {today_date} &nbsp; | &nbsp; ⏱️ 5 Min Read</p>
        </div>
    ...
    <footer>
        ...
        <p>&copy; {current_year} Digital Kamai Hub. All Rights Reserved.</p>
    </footer>
    ...
    <script>
        ...
        function speakArticle() {{
            if (isPlaying) {{
                ...
                document.getElementById("speakBtn").innerHTML = "⏹️ बंद करें";
                isPlaying = true;
...
# HOMEPAGE HTML (Premium Grid)
post_links = ""
for p in posts_db:
    img_src = p.get('img', '[https://via.placeholder.com/400x250](https://via.placeholder.com/400x250)')
    post_links += f"""
    <div style="background:var(--card); border-radius:12px; overflow:hidden; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1); transition:0.3s; border:1px solid #e2e8f0;">
        <img src="{img_src}" style="width:100%; height:200px; object-fit:cover;">
        <div style="padding:20px;">
            <p style="color:#64748b; font-size:13px; margin-bottom:10px; font-weight:bold;">{p['date']}</p>
            <h3 style="font-size:20px; line-height:1.4; margin-bottom:15px;"><a href="{p['file']}" style="color:var(--text); text-decoration:none;">{p['title']}</a></h3>
            <a href="{p['file']}" style="color:var(--primary); font-weight:bold; text-decoration:none;">Read More →</a>
        </div>
    </div>"""

home_html = f"""<!DOCTYPE html><html lang="hi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Digital Kamai Hub</title>{premium_css}
<style>.grid-container {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 30px; margin-top: 40px; }}</style></head>
<body>
    <header><a href="index.html" class="logo">🚀 Digital Kamai Hub</a><nav><a href="#">Home</a></nav></header>
    <div class="main-container">
        <h2 style="font-size:32px; color:#1e293b; border-bottom:2px solid #e2e8f0; padding-bottom:10px;">🔥 ताज़ा लेख</h2>
        <div class="grid-container">{post_links}</div>
    </div>
