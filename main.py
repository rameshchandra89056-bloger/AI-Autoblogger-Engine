...
# --- ADVANCED DUAL-AI ENGINE WITH AUTO-FALLBACK ---
def ask_ai(prompt, retries=3):
    for i in range(retries):
        current_key = API_KEYS[i % len(API_KEYS)]
        # Try Flash First
        api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={current_key}"
        try:
            ...
            print(f"⚠️ Google API Flash Error: {e.code}")
            # Agar Flash 404 de raha hai, toh turant Gemini-Pro try karo
            if e.code == 404:
                ...
                try:
                    ...
                    with urllib.request.urlopen(req_pro, timeout=30) as response_pro:
                        text_pro = json.loads(response_pro.read().decode("utf-8"))['candidates'][0]['content']['parts'][0]['text'].strip()
                        if len(text_pro) > 10: return text_pro
                except Exception as pro_e: print(f"⚠️ Gemini Pro failed too: {pro_e}")
            time.sleep(4)
        except Exception as e:
            print(f"⚠️ Google API Unknown Error: {e}")
            time.sleep(4)
    
    # Plan B: HuggingFace Mistral Backup (Non-Gated, 100% Free)
    print("🔄 Switching to HuggingFace Mistral Backup...")
    ...
    if hf_key:
        try:
            ...
            hf_res = requests.post(hf_url, headers={"Authorization": f"Bearer {hf_key}", "Content-Type": "application/json"}, json={"inputs": f"System: Tum ek expert Hindi blogger ho. Sirf HTML output do.\nUser: {prompt}", "parameters": {"max_new_tokens": 1000, "return_full_text": False}}, timeout=30)
            if hf_res.status_code == 200: 
                return hf_res.json()[0].get('generated_text', '').strip()
            else: 
                print(f"⚠️ HF Error: {hf_res.status_code} - {hf_res.text}")
...
try:
    ...
    if not (chunk_1 and chunk_2 and chunk_3):
        raise Exception("API Timeout: Chunking aur Backup dono fail ho gaye. System ko retry karna padega.")
