...
affiliate_offers = [
    ...
    {"title": "🤖 2026 mein apni kamai ko 10X karein!", "desc": "The AI Millionaire ki exclusive community se judein.", "btn": "👉 Community Join Karein 👈", "link": "#"}
]
...
for idx, mod in enumerate(modifiers):
    if "[PHOTO]" in blog_content:
        ...
        img_tag = f"<div style='text-align: center;'><img src='{inner_img_url}' onerror=\"this.onerror=null; this.src='{safe_fallback}';\" class='article-img' style='width: 100%; border-radius: 12px; margin: 35px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.15); object-fit: cover;'></div>"
        blog_content = blog_content.replace("[PHOTO]", img_tag, 1)
...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, sans-serif; }
    ...
    @media (max-width: 768px) {
        ...
        .timeline-content img { height: 120px; }
    }
...
<footer style="margin-top: 40px; background: #111; padding: 40px 20px; text-align: center;">
    ...
    <div class="footer-links" style="margin-bottom: 20px;">
        <a href="about.html">About Us</a> | <a href="privacy.html">Privacy Policy</a> | <a href="terms.html">Terms</a> | <a href="disclaimer.html">Disclaimer</a> | <a href="contact.html">Contact</a>
    </div>
...
# ==========================================
# 6. ARTICLE PAGE GENERATOR (TTS, NEWSLETTER, SHARE BTN)
# ==========================================
...
<body>
    ...
    <script>
        ...
        function toggleAudio() {{
            ...
            if (audio.paused) {{ audio.play(); btn.innerHTML = "⏸️ Pause Audio"; }} 
            else {{ audio.pause(); btn.innerHTML = "▶️ Play Audio"; }}
        }}
    </script>
    {header_html}
    <div class="container">
        <div class="article-box">
            ...
            <div style="background: linear-gradient(135deg, #f9f9f9, #ffffff); padding: 30px; border-radius: 12px; border: 2px dashed #da251c; margin-top: 40px; text-align: center; box-shadow: 0 10px 30px rgba(218, 37, 28, 0.1);">
                ...
                <p style="color: #555; font-size: 16px; margin-bottom: 20px;">'The AI Millionaire' ki VIP list join karein aur rozana 10X kamai ke naye tarike seekhein (Free).</p>
                <form action="https://formsubmit.co/ajax/rameshchandra89056@gmail.com" method="POST" style="display: flex; gap: 10px; max-width: 500px; margin: 0 auto; flex-wrap: wrap; justify-content: center;">
                    <input type="hidden" name="_subject" value="Naya VIP Subscriber: Digital Kamai Hub!">
                    <input type="hidden" name="_autoresponse" value="Digital Kamai Hub mein swagat hai! Jaldi hi naye hacks milenge.">
                    <input type="email" name="email" placeholder="Apna Email likhein..." required style="flex: 1; padding: 12px; border: 1px solid #ccc; border-radius: 8px; font-size: 16px; min-width: 200px;">
                    <button type="submit" style="background: #111; color: white; border: none; padding: 12px 25px; font-weight: bold; border-radius: 8px; font-size: 16px; cursor: pointer;">Join Now 🚀</button>
                </form>
            </div>
...
# ==========================================
# 7. SMART CATEGORIZATION & TIMELINE PAGES
# ==========================================
...
# ==========================================
# 8. ALL LEGAL PAGES FOR ADSENSE (WITH CONTACT JS)
# ==========================================
pages = {
    ...
    <div style="text-align: center;">
        <h1 style="color: #da251c; font-size: 32px; font-weight: bold; margin-bottom: 10px;">Contact Us</h1>
        <p style="margin-bottom: 30px; font-size: 18px; color: #555;">Humse sampark karein! Apne sawal ya inquiry form ke madhyam se bhejein.</p>
        <div id="form-container" style="max-width: 550px; margin: 0 auto; text-align: left; background: #fff; padding: 30px; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-top: 5px solid #da251c;">
            <form id="my-contact-form" action="https://formsubmit.co/ajax/rameshchandra89056@gmail.com" method="POST">
                <input type="text" name="_honey" style="display:none">
                <input type="hidden" name="_captcha" value="false">
                <input type="text" name="name" placeholder="Aapka Naam" required style="width: 100%; padding: 15px; margin-bottom: 15px; border: 1px solid #ccc; border-radius: 8px; font-size: 16px;">
                <input type="email" name="email" placeholder="Aapka Email" required style="width: 100%; padding: 15px; margin-bottom: 15px; border: 1px solid #ccc; border-radius: 8px; font-size: 16px;">
                <textarea name="message" rows="5" placeholder="Apna Sandesh (Message) likhein..." required style="width: 100%; padding: 15px; margin-bottom: 20px; border: 1px solid #ccc; border-radius: 8px; font-size: 16px;"></textarea>
                <button type="submit" id="submit-btn" style="width: 100%; background: #da251c; color: white; padding: 15px; font-weight: bold; font-size: 18px; border: none; border-radius: 8px; cursor: pointer;">Bhejein (Send Message)</button>
            </form>
            <div id="success-message" style="display:none; text-align: center; padding: 20px; background: #e8f5e9; border: 2px solid #4caf50; border-radius: 8px; margin-top: 10px;">
                <h3 style="color: #4caf50; margin-top:0; margin-bottom: 10px;">✅ Message Bhej Diya Gaya!</h3>
                <p style="color: #333; font-size: 16px; margin: 0;">Dhanyawad! Hum 24-48 ghante ke andar aapse sampark karenge.</p>
            </div>
        </div>
    </div>
    <script>
        ...
        form.addEventListener('submit', function(e) {
            ...
            btn.innerText = 'Bheja ja raha hai...'; btn.disabled = true;
            fetch(form.action, { method: 'POST', body: new FormData(form), headers: { 'Accept': 'application/json' } })
            .then(response => response.json())
            .then(data => {
                if(data.success === 'true' || data.success === true) {
                    form.style.display = 'none'; document.getElementById('success-message').style.display = 'block';
                } else {
                    alert('Error: Kripya dhyan dein ki aapne apna Gmail (FormSubmit) activate kiya hai ya nahi!');
                    btn.innerText = 'Bhejein (Send Message)'; btn.disabled = false;
                }
            }).catch(error => {
                alert('Network Error! Kripya internet check karein.');
                btn.innerText = 'Bhejein (Send Message)'; btn.disabled = false;
            });
        });
    </script>
...
# ==========================================
# 9. SITEMAP GENERATOR & ALERTS (ZERO-TOUCH)
# ==========================================
