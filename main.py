...
def ask_ai(prompt, retries=3):
    ...
    try:
        ...
        res_poll = requests.post(poll_url, json={"messages": [{"role": "system", "content": "Tum ek expert Hindi blogger ho. 1000 shabdon ka lamba article HTML mein do."}, {"role": "user", "content": prompt}], "model": "openai"}, timeout=40)
        if res_poll.status_code == 200:
            text_poll = res_poll.text.strip()
            if len(text_poll) > 100: return clean_ad_garbage(text_poll)
