...
def ask_ai(prompt, retries=2):
    ...
    try:
        ...
        poll_payload = {
            "messages": [
                {"role": "system", "content": "Tum ek expert Hindi blogger ho. Lamba aur detail mein HTML code output do. markdown tags mat lagana."},
...
try:
    ...
    raw_topic = ask_ai(f"Tum ek trend analyst ho. {current_year} mein 'Finance' ya 'AI se online kamai' par ek viral Hindi blog title do. Purane titles se alag ho. Sirf Title likhna.")
    ...
    # --- FORCING 1000+ WORDS IN PROMPTS ---
    intro_prompt = f"Topic: '{current_topic}'. Ek bohot hi lamba aur damdar Introduction (kam se kam 250 shabdon ka) likho. Ek real-life kahani se shuru karo. Dost ki tarah asaan bhasha mein samjhao. Uske baad ek TOC (Table of Contents) likho. Template for TOC: <div style='background: #fffafa; border-left: 5px solid #da251c; padding: 20px; border-radius: 8px; margin-bottom: 25px;'><h3 style='color: #da251c; margin-top: 0;'>📍 Is Article Mein Kya Hai:</h3><ul><li>👉 Point 1</li><li>👉 Point 2</li></ul></div>. Sirf HTML code do (<h2>, <p>). Markdown tags (```html) mat lagana."
    chunk_1 = ask_ai(intro_prompt, retries=2)
    
    body_prompt = f"Topic: '{current_topic}'. Ab article ki main body likho. 3 detailed sub-headings (<h2>) likho. Niyam 1: Har sub-heading ke neeche kam se kam 250 shabdon (words) ka lamba paragraph (<p>) likho taaki total article 1000 shabdon ka ho. Chote paragraph nahi chalenge, vistar (detail) mein likho. Niyam 2: Har <h2> wale section ke baad exactly 1 baar [PHOTO] tag likho (kul 2 [PHOTO] tags hone chahiye). Sirf HTML code do."
    chunk_2 = ask_ai(body_prompt, retries=2)
    
    conclusion_prompt = f"Topic: '{current_topic}'. Ab ek lamba Conclusion (150 shabdon ka) aur 3 FAQs likho. Pura HTML format mein. Beech mein exactly 2 baar [AFFILIATE] tag likho. Markdown tags mat lagana."
    chunk_3 = ask_ai(conclusion_prompt, retries=2)

    if not (chunk_1 and chunk_2 and chunk_3): raise Exception("Chunking fail ho gayi.")
...
<div style="display: flex; gap: 10px; margin-bottom: 20px;">
    ...
    <button onclick="window.open('[https://api.whatsapp.com/send?text=Digital](https://api.whatsapp.com/send?text=Digital) Kamai Hub: ' + window.location.href, '_blank')" style="flex: 1; background: #25D366; color: white; border: none; padding: 10px; border-radius: 5px; font-size: 14px; font-weight: bold; cursor: pointer;">💬 WhatsApp</button>
    <button onclick="window.open('[https://t.me/share/url?url=](https://t.me/share/url?url=)' + window.location.href + '&text=Digital Kamai Hub!', '_blank')" style="flex: 1; background: #0088cc; color: white; border: none; padding: 10px; border-radius: 5px; font-size: 14px; font-weight: bold; cursor: pointer;">✈️ Telegram</button>
</div>
"""

# --- JSON/AJAX FORM FIX WITH NO REDIRECT ---
ajax_form_html = """
<div style="background: linear-gradient(135deg, #f9f9f9, #ffffff); padding: 30px; border-radius: 12px; border: 2px dashed #da251c; margin-top: 40px; text-align: center; box-shadow: 0 10px 30px rgba(218, 37, 28, 0.1);">
...
document.getElementById('ajax-vip-form').addEventListener('submit', function(e) {
    ...
    btn.innerText = 'Wait...';
    btn.disabled = true;
    fetch('[https://formsubmit.co/ajax/rameshchandra89056@gmail.com](https://formsubmit.co/ajax/rameshchandra89056@gmail.com)', {
        method: 'POST',
