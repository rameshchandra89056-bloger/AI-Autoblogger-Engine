...
# --- ADVANCED MULTI-LAYER AI ENGINE (GOD MODE) ---
def ask_ai(prompt, retries=2):
    # Plan A: Google Gemini (Stable v1 Endpoint)
    ...
    # Plan B: GROQ API (Lightning Fast Llama-3)
    groq_key = os.environ.get("GROQ_API_KEY", "").strip()
    if groq_key:
        print("🔄 Switching to GROQ Backup (Llama-3)...")
        try:
            groq_url = "https://api.groq.com/openai/v1/chat/completions"
            groq_payload = {
                "model": "llama3-8b-8192",
                "messages": [
                    {"role": "system", "content": "Tum ek expert Hindi blogger aur storyteller ho. Sirf HTML output do."},
                    {"role": "user", "content": prompt}
                ]
            }
            req_groq = urllib.request.Request(groq_url, data=json.dumps(groq_payload).encode("utf-8"), headers={"Content-Type": "application/json", "Authorization": f"Bearer {groq_key}"})
            with urllib.request.urlopen(req_groq, timeout=30) as res_groq:
                text_groq = json.loads(res_groq.read().decode("utf-8"))['choices'][0]['message']['content'].strip()
                if len(text_groq) > 10: return text_groq
        except Exception as e: print(f"⚠️ GROQ Error: {e}")

    # Plan C: HuggingFace Fallback (Tiny Model guarantee to be loaded)
    print("🔄 Switching to HuggingFace Zephyr Backup...")
    ...
    if hf_key:
        try:
            ...
            hf_res = requests.post(hf_url, headers={"Authorization": f"Bearer {hf_key}", "Content-Type": "application/json"}, json={"inputs": f"<|system|>\nTum ek expert Hindi blogger ho. Sirf HTML output do.</s>\n<|user|>\n{prompt}</s>\n<|assistant|>\n", "parameters": {"max_new_tokens": 1000, "return_full_text": False}}, timeout=30)
