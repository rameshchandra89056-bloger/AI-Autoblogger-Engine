...
# --- SMART ROUND-ROBIN CATEGORY LOGIC (Kabhi topic repeat nahi hoga) ---
categories_list = ["AI", "Trading", "Finance"]
# Ye decide karega ki aaj kiski baari hai
todays_category = categories_list[len(posts_db) % len(categories_list)]
...
try:
    ...
    # 1. STRICT TOPIC GENERATION (Based on Round Robin)
...
for idx, mod in enumerate(["cinematic", "cyberpunk", "hyperrealistic"]):
    if "[PHOTO]" in blog_content:
        ...
        # SPEED HACK: loading='lazy' added for 0.1sec load time
...
# Saving the explicit category in DB
...
# --- AGGRESSIVE JS (Works perfectly even if user scrolls just a bit) ---
...
for p in posts_db: 
    # Use explicitly saved category if available, else fallback
...
send_telegram_msg(urllib.parse.quote(f"✅ ROUND-ROBIN SUCCESS: Naya Blog Live!\n📝 Kul Post: {len(posts_db)}\n🌐 Link: {blog_url}"))
