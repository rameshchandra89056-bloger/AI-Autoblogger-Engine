...
def ask_ai(prompt, retries=3):
    for i in range(retries):
        ...
        except Exception: time.sleep(5) # API Rate limit ko reset hone ka time
...
try:
    ...
    raw_topic = ask_ai(f"Tum ek trend analyst ho. Aaj ki category '{todays_category}' hai. Mujhe {current_year} ke liye '{todays_category}' niche par ek bohot hi viral Hindi blog title do. Purane titles se bilkul alag ho. Sirf mukhya Title likhna.")
    ...
    time.sleep(8) # SMART WORK: Rate Limit Reset
    
    print("⏳ Chunk 2 generating...")
    body_prompt = f"Topic: '{current_topic}'. Ab main body likho. 3 sub-headings (<h2>) likho. Niyam 1: Pehle <h2> mein id='basic' lagao. Dusre mein id='deep' lagao. Teesre mein id='pro' lagao. Niyam 2: Bhasha dostana aur desi rakho. Niyam 3: Kam se kam 3 asali Tools/Websites ke naam do aur step-by-step use karna batao. Niyam 4: Har <h2> wale section ke baad exactly 1 baar [PHOTO] tag likho. Sirf HTML code do."
...
send_telegram_msg(urllib.parse.quote(f"✅ UNBREAKABLE FIX SUCCESS: Naya Blog Live!\n📝 Kul Post: {len(posts_db)}\n🌐 Link: {blog_url}"))
